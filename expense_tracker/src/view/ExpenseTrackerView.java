...
public class ExpenseTrackerView extends JFrame {
  ...
  public ExpenseTrackerView() {
    ...
    String[] columnNames = { "serial", "Amount", "Category", "Date", "Undo" };
    this.model = new DefaultTableModel(columnNames, 0);

    // Create table
    transactionsTable = new JTable(model);
    transactionsTable.getColumn("Undo").setCellEditor(null);
  ...
  // Adds Undo buttons to each transaction in the UI and injects event listener.
  public void setupUndo(IntConsumer undoCallback) {
    transactionsTable.getColumn("Undo").setCellRenderer(new ButtonRenderer(this.model));
    transactionsTable.getColumn("Undo").setCellEditor(new ButtonEditor(transactionsTable, undoCallback));
